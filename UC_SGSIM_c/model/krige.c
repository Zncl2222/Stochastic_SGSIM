...
void SimpleKrige(double* array, double* sampled, double* u_array, int currlen, double unsampled_point, int idx ,int neighbor, int randomseed){

    if(neighbor == 0){

        array[(int)unsampled_point] = random_normal(randomseed);
    }
    else{
        ...
        if(close == 0){
            ...
            if(svar < 0)
                svar = 0;
            fix = random_normal(randomseed) * pow(svar, 0.5);
