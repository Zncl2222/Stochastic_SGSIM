...
void sgsim(int X, int nR, int hs, int bw, double range, double sill, int randomseed, int vario_flag){
    ...
    while (count < nR){
        ...
        x_grid = randompath(x_grid, X, randomseed);
        ...
        for(int i = 0; i < X; i++){
            ...
            SimpleKrige(sgsim_array, sampled, u_array, currentlen, x_grid[i], i, neighbor, randomseed);
        ...
        if(flag > 0)
            ...
            save_1darray(sgsim_array, X,"Realizations", "./Realizations/" ,count);
            if(vario_flag ==1)
                save_1darray(variogram_array, hs,"Variogram", "./Realizations/Variogram/", count);
        }
    }

}

void sgsim_dll(double* RandomFieldX, int X, int nR, double range, double sill, int randomseed){
